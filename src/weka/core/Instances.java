...
RevisionHandler {
   ...
   */
  	...
  	**/
  	...
  	public void add(Instances insts){
   ...
   */
    ...
    public void setClassIndex(String className){
    	setClassIndex(attribute(className).index());
    }
  /**
   ...
   */
    ...
    public Instances attributeFilter(int[] features) throws Exception{
    	Remove delTransform = new Remove();
    	Instances trainCopy = new Instances(this);
    	delTransform.setInvertSelection(true);
    	delTransform.setAttributeIndicesArray(features);
    	delTransform.setInputFormat(trainCopy);
    	trainCopy = Filter.useFilter(trainCopy, delTransform);
    	for(int i:features)
    		if (i==classIndex()) {
    			trainCopy.setClassIndex(classAttribute().name());
    			break;
			}
    	return trainCopy;
    }
    
    public Instances attributeFilter(BitSet bitFeatures) throws Exception{
    	int i,j;
    	for(i=0,j=0;i<bitFeatures.length();++i)
    		if(bitFeatures.get(i))
    			++j;
    	if(j==0)
    		throw new Exception("J can't be zero!");
    	int[] features=new int[j];
    	for(i=0,j=0;i<bitFeatures.length();++i)
    		if(bitFeatures.get(i))
    			features[j++]=i;
    	return attributeFilter(features);
    }
}
